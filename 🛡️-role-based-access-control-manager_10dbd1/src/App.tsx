...
type GroupType = 'internal' | 'external' | 'application';
...
const getGroupTypeColor = (type: GroupType): string => {
  switch (type) {
    case 'internal':
      return 'badge badge-info';
    case 'external':
...
const App: React.FC = () => {
  ...
  // Current view state
  const [currentView, setCurrentView] = useState<'users' | 'groups' | 'permissions' | 'appGroups'>('users');
  ...
  const [appGroupModalOpen, setAppGroupModalOpen] = useState<boolean>(false);
  const [permissionModalOpen, setPermissionModalOpen] = useState<boolean>(false);
  const [currentUser, setCurrentUser] = useState<User | null>(null);
  const [currentGroup, setCurrentGroup] = useState<Group | null>(null);
  const [currentAppGroup, setCurrentAppGroup] = useState<Group | null>(null);
  ...
  function generateSampleGroups(): Group[] {
    return [
      ...
      },
      {
        ...
        description: 'Users who access the customer portal application',
        type: 'application',
        permissions: ['2'],
        members: ['1', '2'],
        createdAt: new Date(2023, 5, 15).toISOString(),
        app_id: 'customer-portal-123'
      },
      {
        id: '7',
        name: 'Analytics Dashboard Users',
        description: 'Users who can access the analytics dashboard application',
        type: 'application',
        permissions: ['2'],
        members: ['3'],
        createdAt: new Date(2023, 5, 20).toISOString(),
        app_id: 'analytics-dash-456'
      }
    ];
  }
  ...
  const getFilteredGroups = () => {
    // Filter for regular groups (internal/external, not application)
    let result = [...groups].filter(group => group.type !== 'application');
    ...
    // Apply type filter
    if (filter.groupType && filter.groupType !== 'application') {
  ...
  const getFilteredAppGroups = () => {
    // Filter for application groups only
    let result = [...groups].filter(group => group.type === 'application');
    ...
    if (searchTerm) {
      ...
      result = result.filter(group => 
        group.name.toLowerCase().includes(term) || 
        group.description.toLowerCase().includes(term) ||
        (group.app_id?.toLowerCase() || '').includes(term)
      );
    }
    
    // Apply app_id filter
    if (filter.appId) {
      result = result.filter(group => group.app_id === filter.appId);
    }
  ...
  const getCurrentItems = () => {
    ...
    switch (currentView) {
      ...
      case 'groups':
        ...
        break;
      case 'appGroups':
        filteredItems = getFilteredAppGroups();
        break;
      case 'permissions':
  ...
  const getTotalPages = () => {
    ...
    switch (currentView) {
      ...
      case 'groups':
        ...
        break;
      case 'appGroups':
        totalItems = getFilteredAppGroups().length;
        break;
      case 'permissions':
  ...
  const getGroupTypeById = (id: string): GroupType | undefined => {
    const group = groups.find(g => g.id === id);
    return group?.type;
  };
  ...
  // Get unique app_ids for filtering
  const getUniqueAppIds = (): string[] => {
    const appIds = groups
      .filter(g => g.type === 'application' && g.app_id)
      .map(g => g.app_id as string);
  ...
  // Helper for creating a regular group form submission
  ...
  // Helper for creating/updating an application group
  const handleAppGroupFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    if (currentAppGroup) {
      ...
      addGroup(appGroupData);
    }
    
    setCurrentAppGroup(null);
    setAppGroupModalOpen(false);
  };
    ...
    <div className="min-h-screen flex flex-col theme-transition-all">
      ...
      <nav className="bg-gray-100 dark:bg-slate-700 p-4">
        <div className="container-fluid flex flex-wrap gap-2">
          ...
          <button 
            className={`btn ${currentView === 'appGroups' ? 'btn-primary' : 'bg-white dark:bg-slate-800 text-gray-700 dark:text-white'} flex items-center gap-2`}
            onClick={() => { setCurrentView('appGroups'); resetFilters(); }}
          >
            <Globe size={18} />
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
                ...
                {currentView === 'appGroups' && 'Application Group Management'}
                {currentView === 'permissions' && 'Permission Management'}
              </h2>
              
              {currentView === 'appGroups' && (
                <div className="ml-2 text-sm text-gray-600 dark:text-slate-400">
                  <p>Map users and groups to third-party applications</p>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <div className="relative">
                <input
                  type="text"
                  placeholder={`Search ${currentView === 'appGroups' ? 'application groups' : currentView}...`}
                  value={searchTerm}
                  ...
                  {currentView === 'appGroups' && (
                    <div className="p-2">
                      ...
                      <select 
                        ...
                        value={filter.appId || ''}
                        onChange={(e) => {
                          setFilter({ ...filter, appId: e.target.value || undefined });
                          setCurrentPage(1);
                        }}
                      >
                        <option value="">All Applications</option>
                        {getUniqueAppIds().map(appId => (
              ...
              <button 
                ...
                onClick={() => {
                  ...
                  } else if (currentView === 'appGroups') {
                    setCurrentAppGroup(null);
                    setAppGroupModalOpen(true);
              ...
              >
                <Plus size={16} />
                <span>Add New {currentView === 'appGroups' ? 'App Group' : currentView.slice(0, -1)}</span>
              </button>
            </div>
          </div>
                            ...
                            <div className="flex flex-wrap gap-1">
                              {user.groups.length > 0 ? (
                                user.groups.map(groupId => {
                                  const groupType = getGroupTypeById(groupId);
                                  return (
                                    <span key={groupId} className={getGroupTypeColor(groupType || 'internal')}>
                    ...
                    {getCurrentItems().length === 0 ? (
                      ...
                      getCurrentItems().map((group: Group) => (
                        <tr key={group.id} className="hover:bg-gray-50 dark:hover:bg-slate-700">
                          ...
                          <td className="table-cell px-4 py-4">
                            <span className={getGroupTypeColor(group.type)}>
                              {group.type}
                            </span>
                          </td>
                          <td className="table-cell px-4 py-4">
                ...
                <table className="table">
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
                    {getCurrentItems().length === 0 ? (
                      <tr>
                        <td colSpan={7} className="table-cell text-center py-8 text-gray-500 dark:text-slate-400">
                          No application groups found. Try a different search or add a new application group.
                        </td>
                      </tr>
                    ) : (
                      getCurrentItems().map((group: Group) => (
                          ...
                          <td className="table-cell px-4 py-4">
                            <div className="flex items-center gap-2">
                              <button 
                                ...
                                aria-label="Edit application group"
                              >
                                <Edit size={16} />
                              </button>
                              <button 
                                className="p-1 text-red-600 hover:bg-red-100 rounded-full dark:text-red-400 dark:hover:bg-slate-700"
                                onClick={() => deleteGroup(group.id)}
                                aria-label="Delete application group"
                              >
                                <Trash2 size={16} />
                              </button>
                            </div>
                ...
                <div className="text-sm text-gray-500 dark:text-slate-400">
                  ...
                  <span className="font-medium">
                    {Math.min(currentPage * itemsPerPage, {
                      ...
                      'appGroups': getFilteredAppGroups().length,
          ...
          <div className="form-group">
            ...
            <div className="mt-1 space-y-4">
              ...
              <div>
                ...
                <div className="max-h-28 overflow-y-auto p-2 border border-gray-300 dark:border-slate-600 rounded-md space-y-2">
                  {groups
                  ...
                  {groups
                    ...
                    .map(group => (
                      <div key={group.id} className="flex items-center">
                        ...
                        />
                        <label htmlFor={`group-${group.id}`} className="ml-2 text-sm text-gray-700 dark:text-slate-300">
                          {group.name} <span className="text-gray-400 dark:text-slate-500">({group.app_id})</span>
                        </label>
                      </div>
                    ))
                  }
      ...
      {/* Application Group Modal */}
      <Modal 
        isOpen={appGroupModalOpen} 
        onClose={() => { setAppGroupModalOpen(false); setCurrentAppGroup(null); }}
        title={currentAppGroup ? 'Edit Application Group' : 'Add New Application Group'}
      >
        <form onSubmit={handleAppGroupFormSubmit} className="space-y-4">
          <div className="form-group">
            <label className="form-label" htmlFor="app_name">Group Name</label>
            <input 
              id="app_name" 
              name="name" 
              type="text" 
              className="input" 
              defaultValue={currentAppGroup?.name || ''}
              placeholder="Marketing Dashboard Users"
              required 
            />
          </div>
          
          <div className="form-group">
            <label className="form-label" htmlFor="app_id">Application ID</label>
            <input 
              id="app_id" 
              name="app_id" 
              type="text" 
              className="input" 
              defaultValue={currentAppGroup?.app_id || ''}
              placeholder="marketing-dashboard-123"
              list="app_ids"
              required 
            />
            <datalist id="app_ids">
            ...
            <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">
              The unique identifier for the application (e.g., customer-portal-123)
            </p>
          </div>
          
          <div className="form-group">
            <label className="form-label" htmlFor="app_description">Description</label>
            <textarea 
              id="app_description" 
              name="description" 
              className="input" 
              defaultValue={currentAppGroup?.description || ''}
              placeholder="Users who can access the marketing dashboard application"
              rows={2}
              required 
            />
          </div>
            ...
            <div className="mt-1 space-y-2 max-h-40 overflow-y-auto p-2 border border-gray-300 dark:border-slate-600 rounded-md">
              {permissions.map(permission => (
                <div key={permission.id} className="flex items-center">
                  <input
                    type="checkbox"
                    id={`app-permission-${permission.id}`}
                    name="permissions"
                    value={permission.id}
                    defaultChecked={currentAppGroup?.permissions.includes(permission.id)}
                    className="h-4 w-4 text-primary-600 rounded border-gray-300 focus:ring-primary-500"
                  />
                  <label htmlFor={`app-permission-${permission.id}`} className="ml-2 text-sm text-gray-700 dark:text-slate-300">
                    {permission.name} <span className="text-gray-400 dark:text-slate-500">({permission.resource})</span>
                  </label>
                </div>
              ))}
            ...
            <div className="mt-1 space-y-2 max-h-40 overflow-y-auto p-2 border border-gray-300 dark:border-slate-600 rounded-md">
              {users.map(user => (
                <div key={user.id} className="flex items-center">
                  <input
                    type="checkbox"
                    id={`app-member-${user.id}`}
                    name="members"
                    value={user.id}
                    defaultChecked={currentAppGroup?.members.includes(user.id)}
                    className="h-4 w-4 text-primary-600 rounded border-gray-300 focus:ring-primary-500"
                  />
                  <label htmlFor={`app-member-${user.id}`} className="ml-2 text-sm text-gray-700 dark:text-slate-300">
                    {user.name} <span className="text-gray-400 dark:text-slate-500">({user.email})</span>
                  </label>
                </div>
              ))}
          ...
          <div className="modal-footer">
            <button 
              type="button" 
              className="btn bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600"
              onClick={() => { setAppGroupModalOpen(false); setCurrentAppGroup(null); }}
            >
              Cancel
            </button>
            <button type="submit" className="btn btn-primary">
              {currentAppGroup ? 'Update' : 'Create'}
            </button>
          </div>
        </form>
      </Modal>
