            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg border border-slate-200 dark:border-slate-700">
                  <h3 className="text-lg font-semibold text-[#1F2E3D] dark:text-[#e7f7f7] mb-4">Priority Distribution</h3>
                  <div className="space-y-3">
                    {Object.entries(stats.byPriority).map(([priority, count]) => (
                      <div key={priority} className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className={`w-3 h-3 rounded-full ${getPriorityColor(priority).replace('text-', 'bg-').split(' ')[1]}`}></div>
                          <span className="text-sm text-slate-600 dark:text-slate-400 capitalize">{priority.replace('_', ' ')}</span>
                        </div>
                        <span className="text-sm font-semibold text-[#1F2E3D] dark:text-[#e7f7f7]">{count}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg border border-slate-200 dark:border-slate-700">
                  <h3 className="text-lg font-semibold text-[#1F2E3D] dark:text-[#e7f7f7] mb-4">Category Distribution</h3>
                  <div className="space-y-3">
                    {Object.entries(stats.byCategory).map(([category, count]) => (
                      <div key={category} className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className="w-3 h-3 rounded-full bg-[#548b99]"></div>
                          <span className="text-sm text-slate-600 dark:text-slate-400 capitalize">{category.replace('_', ' ')}</span>
                        </div>
                        <span className="text-sm font-semibold text-[#1F2E3D] dark:text-[#e7f7f7]">{count}</span>
                      </div>
                    ))}
              ...
              <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg border border-slate-200 dark:border-slate-700">
                ...
                <div className="space-y-3">
                  {filteredTickets.slice(0, 5).map((ticket) => (
                    <div key={ticket.id} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-700 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-600 transition-all cursor-pointer">
                      <div className="flex-1">
                        <h4 className="font-medium text-[#1F2E3D] dark:text-[#e7f7f7]">{ticket.title}</h4>
                        <p className="text-sm text-slate-600 dark:text-slate-400">{ticket.customerName}</p>
                      </div>
                      <div className="flex items-center gap-3">
