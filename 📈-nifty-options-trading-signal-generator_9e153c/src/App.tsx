...
const App: React.FC = () => {
  ...
  const [settings, setSettings] = useState<Settings>({
    ...
    darkMode: true // Default to dark mode for Kraken-like UI
  });
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-slate-900 text-white">
        ...
        <h2 className="mt-6 text-xl font-medium">Loading NIFTY Analyzer</h2>
        <p className="mt-2 text-gray-400">Connecting to market data sources...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-slate-900 text-white">
        ...
        <h2 className="mt-6 text-xl font-medium">Error Loading Data</h2>
        <p className="mt-2 text-gray-400">{error}</p>
        <button 
          className="mt-6 btn bg-blue-600 text-white hover:bg-blue-700"
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-900 text-white">
        ...
        <div className="container-fluid">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center">
              <img 
                src="/logo.png" 
                alt="NIFTY Options Analyzer Logo" 
                className={styles.headerLogo}
                width={32}
                height={32} 
              />
              <div className="ml-3">
                <h1 className="text-xl font-medium text-white">NIFTY Options Analyzer</h1>
              </div>
            </div>
      ...
      <main className="flex-grow container-fluid py-6">
        ...
        <div className={`${styles.liveBanner} mb-6 p-3 rounded-lg bg-blue-900 text-white flex items-center justify-between`}>
          <div className="flex items-center">
          ...
          <button 
            className="text-white bg-blue-800 hover:bg-blue-700 transition px-3 py-1 rounded-md text-sm flex items-center"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div className="bg-slate-800 rounded-lg p-5 border border-slate-700">
            <div className="flex flex-col">
              <h3 className="text-base font-medium text-gray-400">NIFTY Current Value</h3>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${accuracy >= 80 ? 'bg-green-900 text-green-300' : accuracy >= 70 ? 'bg-yellow-900 text-yellow-300' : 'bg-red-900 text-red-300'}`}>
                ...
                <span className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-green-900 text-green-300">
          ...
          {/* Daily summary */}
          <div className="bg-slate-800 rounded-lg p-5 border border-slate-700">
            <h3 className="text-base font-medium text-gray-400">Market Summary</h3>
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div>
                <p className="text-sm text-gray-400">Daily Range</p>
              ...
              <div>
                ...
                <p className="font-medium text-white">14.2M</p>
              </div>
              <div>
          ...
          <div className="bg-slate-800 rounded-lg p-5 border border-slate-700">
            <h3 className="text-base font-medium text-gray-400">Technical Indicators</h3>
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div>
                <p className="text-sm text-gray-400">RSI (14)</p>
                <p className={`font-medium ${marketData.length && marketData[marketData.length - 1].rsi! > 70 ? 'text-red-400' : marketData.length && marketData[marketData.length - 1].rsi! < 30 ? 'text-green-400' : 'text-white'}`}>
                  {marketData.length ? marketData[marketData.length - 1].rsi!.toFixed(2) : '-'}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-400">MACD</p>
                <p className={`font-medium ${marketData.length && marketData[marketData.length - 1].macd! > marketData[marketData.length - 1].signal! ? 'text-green-400' : 'text-red-400'}`}>
                ...
                <div className="flex items-center">
                  {marketData.length > 5 && marketData[marketData.length - 1].value > marketData[marketData.length - 5].value ? (
                    <>
                      ...
                      <p className="font-medium text-green-400">Bullish</p>
                    </>
                  ) : (
                    <>
                      <TrendingDown size={16} className="text-red-400 mr-1" />
                      <p className="font-medium text-red-400">Bearish</p>
                    </>
                  )}
                </div>
              </div>
        ...
        <div className="bg-slate-800 rounded-lg p-5 border border-slate-700 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6">
            <h2 className="text-xl font-medium text-white mb-3 sm:mb-0">NIFTY Price Chart</h2>
            ...
            <div className="flex items-center space-x-2">
              <div className="flex bg-slate-700 rounded-md overflow-hidden">
                {['1D', '1W', '1M', '3M'].map(timeframe => (
                  <button
                    key={timeframe}
                    className={`px-3 py-1.5 text-sm font-medium ${selectedTimeframe === timeframe ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-slate-600'}`}
        ...
        <div className="bg-slate-800 rounded-lg p-5 border border-slate-700 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4">
            <h2 className="text-xl font-medium text-white mb-3 sm:mb-0">Trading Signals</h2>
            <div className="flex items-center space-x-2">
              <button 
                className="px-3 py-1.5 rounded-md bg-slate-700 text-gray-300 hover:bg-slate-600 transition-colors flex items-center"
          ...
          {/* Filter panel */}
          {isFiltering && (
            <div className="bg-slate-700 p-4 rounded-md mb-4 border border-slate-600">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Signal Type</label>
                  <select 
                    className="w-full bg-slate-800 border border-slate-600 rounded-md px-3 py-2 text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="text-xs font-medium text-gray-400 uppercase border-b border-slate-600">
                <tr>
              ...
              <tbody className="divide-y divide-slate-700">
                {getFilteredSignals().map(signal => (
                  <tr key={signal.id} className="hover:bg-slate-700/50">
                    <td className="px-4 py-4 whitespace-nowrap text-sm">{formatDate(signal.timestamp)}</td>
                    <td className="px-4 py-4 whitespace-nowrap text-sm">
                      <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${signal.type === 'BUY' ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'}`}>
                        {signal.type === 'BUY' ? (
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-sm text-white">{signal.strike}</td>
                    <td className="px-4 py-4 whitespace-nowrap text-sm text-white">{signal.expiryDate}</td>
                    <td className="px-4 py-4 whitespace-nowrap text-sm text-white">₹{signal.premium}</td>
                    <td className="px-4 py-4 whitespace-nowrap text-sm">
                      <div className="flex items-center">
                        <div className="w-full bg-slate-700 rounded-full h-2">
                          <div 
                            className={`h-2 rounded-full ${signal.confidence >= 85 ? 'bg-green-500' : signal.confidence >= 75 ? 'bg-blue-500' : 'bg-yellow-500'}`} 
                            style={{ width: `${signal.confidence}%` }}
                          ></div>
                        </div>
                        <span className="ml-2 text-xs text-gray-300">{signal.confidence}%</span>
                      </div>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap text-sm">
                      <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${signal.status === 'OPEN' ? 'bg-blue-900 text-blue-300' : signal.result === 'PROFIT' ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'}`}>
                        {signal.status === 'OPEN' ? 'OPEN' : signal.result}
                      </span>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap text-sm">
                      {signal.status === 'CLOSED' && (
                        <span className={signal.profitLoss! >= 0 ? 'text-green-400' : 'text-red-400'}>
                          {signal.profitLoss! >= 0 ? '+' : ''}
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-sm">
                      <button 
                        className="p-1 rounded-full bg-slate-700 hover:bg-slate-600 transition-colors text-blue-400 hover:text-blue-300"
        ...
        <div className="bg-slate-800 rounded-lg p-5 border border-slate-700">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6">
            <h2 className="text-xl font-medium text-white mb-3 sm:mb-0">Options Chain</h2>
            <div className="flex items-center">
              <div className="relative">
                <input 
                  type="text" 
                  className="pl-9 pr-3 py-2 bg-slate-700 border border-slate-600 rounded-md text-white placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 w-full sm:w-auto" 
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr>
                  <th colSpan={5} className="text-center bg-blue-900/30 text-blue-300 px-4 py-2 text-xs font-medium uppercase">CALLS</th>
                  <th className="bg-slate-700 text-center px-4 py-2 text-xs font-medium text-gray-300 uppercase">Strike</th>
                  <th colSpan={5} className="text-center bg-red-900/30 text-red-300 px-4 py-2 text-xs font-medium uppercase">PUTS</th>
                </tr>
                <tr>
                  <th className="px-3 py-2 text-xs font-medium text-blue-300 text-left bg-blue-900/20">IV</th>
                  <th className="px-3 py-2 text-xs font-medium text-blue-300 text-left bg-blue-900/20">Volume</th>
                  <th className="px-3 py-2 text-xs font-medium text-blue-300 text-left bg-blue-900/20">OI</th>
                  <th className="px-3 py-2 text-xs font-medium text-blue-300 text-left bg-blue-900/20">Change</th>
                  <th className="px-3 py-2 text-xs font-medium text-blue-300 text-left bg-blue-900/20">Premium</th>
                  <th className="px-3 py-2 text-xs font-medium text-gray-300 text-center bg-slate-700"></th>
                  <th className="px-3 py-2 text-xs font-medium text-red-300 text-left bg-red-900/20">Premium</th>
                  <th className="px-3 py-2 text-xs font-medium text-red-300 text-left bg-red-900/20">Change</th>
                  <th className="px-3 py-2 text-xs font-medium text-red-300 text-left bg-red-900/20">OI</th>
                  <th className="px-3 py-2 text-xs font-medium text-red-300 text-left bg-red-900/20">Volume</th>
                  <th className="px-3 py-2 text-xs font-medium text-red-300 text-left bg-red-900/20">IV</th>
                </tr>
              </thead>
              <tbody>
                ...
                .map(({ strike, call, put }) => (
                  <tr key={strike} className={`${strike === Math.round(currentNifty / 50) * 50 ? 'bg-slate-700/30' : ''} border-b border-slate-700 last:border-0`}>
                    <td className="px-3 py-3 text-xs text-white bg-blue-900/10">{call?.iv || '-'}%</td>
                    <td className="px-3 py-3 text-xs text-white bg-blue-900/10">{call?.volume?.toLocaleString() || '-'}</td>
                    <td className="px-3 py-3 text-xs text-white bg-blue-900/10">{call?.openInterest?.toLocaleString() || '-'}</td>
                    <td className="px-3 py-3 text-xs bg-blue-900/10">
                      <span className={call?.change && call.change >= 0 ? 'text-green-400' : 'text-red-400'}>
                        {call?.change ? (call.change >= 0 ? '+' : '') + call.change + '%' : '-'}
                      </span>
                    </td>
                    <td className="px-3 py-3 text-xs text-white font-medium bg-blue-900/10">
                      ₹{call?.premium.toFixed(2) || '-'}
                    </td>
                    <td className="px-3 py-3 text-xs text-center font-medium text-white bg-slate-700">{strike}</td>
                    <td className="px-3 py-3 text-xs text-white font-medium bg-red-900/10">
                      ₹{put?.premium.toFixed(2) || '-'}
                    </td>
                    <td className="px-3 py-3 text-xs bg-red-900/10">
                      <span className={put?.change && put.change >= 0 ? 'text-green-400' : 'text-red-400'}>
                        {put?.change ? (put.change >= 0 ? '+' : '') + put.change + '%' : '-'}
                      </span>
                    </td>
                    <td className="px-3 py-3 text-xs text-white bg-red-900/10">{put?.openInterest?.toLocaleString() || '-'}</td>
                    <td className="px-3 py-3 text-xs text-white bg-red-900/10">{put?.volume?.toLocaleString() || '-'}</td>
                    <td className="px-3 py-3 text-xs text-white bg-red-900/10">{put?.iv || '-'}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div className="flex justify-center mt-4">
              <select className="mr-4 bg-slate-700 border border-slate-600 rounded-md px-3 py-2 text-white focus:outline-none focus:ring-1 focus:ring-blue-500">
              ...
              <button className="px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md flex items-center transition-colors">
      ...
      {/* Footer */}
      <footer className="bg-slate-800 border-t border-slate-700 py-4 px-4 text-center text-gray-400 text-sm">
        <div className="container-fluid">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50" onClick={closeModal}>
          <div 
            ref={modalRef}
            className="bg-slate-800 rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-auto p-6 m-4 border border-slate-700" 
          ...
          >
            ...
            {modalType === 'settings' && (
              <>
                <div className="flex justify-between items-start mb-4">
                  ...
                  <button className="text-gray-400 hover:text-gray-300" onClick={closeModal}>
                    <X size={24} />
                  </button>
                </div>
                <div className="mt-4 space-y-6">
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">Data Refresh Interval (seconds)</label>
                    <input 
                      type="range" 
                      min="5" 
                      max="60" 
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">Signal Confidence Threshold</label>
                    <input 
                      type="range" 
                      min="50" 
                      max="95" 
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button 
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-300 focus:outline-none ${settings.enableNotifications ? 'bg-blue-600' : 'bg-slate-600'}`}
                      onClick={() => setSettings({...settings, enableNotifications: !settings.enableNotifications})}
                    >
                      <span 
                        className={`absolute h-4 w-4 rounded-full bg-white shadow-lg transition-transform duration-300 ${settings.enableNotifications ? 'translate-x-6' : 'translate-x-1'}`}
                      />
                    </button>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button 
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-300 focus:outline-none ${settings.darkMode ? 'bg-blue-600' : 'bg-slate-600'}`}
                      onClick={() => setSettings({...settings, darkMode: !settings.darkMode})}
                    >
                      <span 
                        className={`absolute h-4 w-4 rounded-full bg-white shadow-lg transition-transform duration-300 ${settings.darkMode ? 'translate-x-6' : 'translate-x-1'}`}
                      />
                    </button>
                  </div>
                </div>
                    ...
                    <div>
                      ...
                      <ul className="divide-y divide-slate-700">
                        {notifications.map(notification => (
                          <li key={notification.id} className={`py-4 ${!notification.read ? 'bg-blue-900/20' : ''}`}>
              ...
              <>
                ...
                <div className="mt-4">
                  <div className="flex justify-between mb-6">
                    <span className={`inline-flex items-center px-3 py-1 rounded-md text-sm font-medium ${selectedSignal.type === 'BUY' ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'}`}>
                    ...
                    <span className={`inline-flex items-center px-3 py-1 rounded-md text-sm font-medium ${selectedSignal.status === 'OPEN' ? 'bg-blue-900 text-blue-300' : selectedSignal.result === 'PROFIT' ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    <div>
                      ...
                      <p className="font-medium text-white">{formatDate(selectedSignal.timestamp)}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-400">Strike Price</p>
                      <p className="font-medium text-white">{selectedSignal.strike}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-400">Expiry Date</p>
                      <p className="font-medium text-white">{selectedSignal.expiryDate}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-400">Premium</p>
                      <p className="font-medium text-white">₹{selectedSignal.premium}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-400">Confidence</p>
                      <p className="font-medium text-white">{selectedSignal.confidence}%</p>
                    </div>
                    {selectedSignal.status === 'CLOSED' && (
                      <div>
                        <p className="text-sm text-gray-400">Profit/Loss</p>
                        <p className={`font-medium ${selectedSignal.profitLoss! >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                  ...
                  <div className="mb-6">
                    <p className="text-sm text-gray-400 mb-1">Signal Notes</p>
                    <div className="bg-slate-700 rounded-md p-3">
                      <p className="text-gray-300">{selectedSignal.notes}</p>
                    </div>
                  </div>

                  <div className="bg-blue-900/20 rounded-md p-4 mb-4">
                ...
                <div className="flex justify-end gap-2 mt-6">
                  <button className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-gray-300 rounded-md transition-colors" onClick={closeModal}>
                    Close
                  </button>
            ...
            {modalType === 'dataSources' && (
              <>
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-lg font-medium text-white">Live Data Sources</h3>
                  <button className="text-gray-400 hover:text-gray-300" onClick={closeModal}>
                    <X size={24} />
                  </button>
                </div>
                <div className="mt-4">
                  <div className="mb-4 p-3 bg-blue-900/20 rounded-md">
                    <div className="flex items-start">
                      <Info size={18} className="text-blue-400 mt-0.5 mr-2 flex-shrink-0" />
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead className="text-xs font-medium text-gray-400 uppercase border-b border-slate-700">
                        <tr>
                          <th className="px-4 py-3 text-left">Source</th>
                          <th className="px-4 py-3 text-left">Status</th>
                          <th className="px-4 py-3 text-left">Trust Score</th>
                          <th className="px-4 py-3 text-left">Last Value</th>
                      ...
                      <tbody className="divide-y divide-slate-700">
                        {dataSources.sort((a, b) => b.trustScore - a.trustScore).map(source => (
                          <tr key={source.id} className={`hover:bg-slate-700/50 ${source.id === activeSource ? 'bg-green-900/20' : ''}`}>
                            ...
                            <td className="px-4 py-3 text-sm">
                              <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${source.status === 'active' ? 'bg-green-900 text-green-300' : source.status === 'error' ? 'bg-red-900 text-red-300' : 'bg-gray-700 text-gray-300'}`}>
                            ...
                            <td className="px-4 py-3 text-sm">
                              <div className="flex items-center">
                                ...
                                <span className="text-xs text-gray-300">{source.trustScore.toFixed(1)}</span>
                              </div>
                            </td>
                  ...
                  <div className="mt-6">
                    <h4 className="text-sm font-medium text-gray-300 mb-2">Recent Activity Logs</h4>
                    <div className="bg-slate-700 rounded-md p-3 max-h-40 overflow-y-auto">
                      {fetchingLogs.slice(0, 10).map((log, index) => (
                        <div key={index} className={`text-xs mb-1 pb-1 ${index < fetchingLogs.length - 1 ? 'border-b border-slate-600' : ''}`}>
                          <span className={`${log.type === 'error' ? 'text-red-400' : log.type === 'warning' ? 'text-yellow-400' : 'text-blue-400'}`}>
                      ...
                      {fetchingLogs.length === 0 && (
                        <p className="text-xs text-gray-400">No activity logs available</p>
                      )}
                    </div>
                  </div>
                </div>
      ...
      {signals.length > 0 && signals[0].status === 'OPEN' && (
        <div className="fixed bottom-4 right-4 left-4 sm:left-auto sm:w-80 bg-slate-800 shadow-lg rounded-lg overflow-hidden border border-blue-500 z-50">
          <div className="p-4">
            <div className="flex items-start justify-between">
              <div className="flex items-center">
                <Zap size={18} className="text-yellow-400 mr-2" />
                <h3 className="text-sm font-medium text-white">New Trading Signal</h3>
              </div>
