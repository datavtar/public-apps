...
import { AILayerProps, AuthTokenResponse, AILayerHandle } from './AILayer.types';
...
const stripMarkdownFences = (responseString: string): string => {
  const trimmedResponse = responseString.trim();
  // Regex to capture content within ```json ... ``` or ``` ... ```
  // Handles optional "json" identifier and varying whitespace/newlines around the content
  const fenceRegex = /^```(?:json)?\s*([\s\S]*?)\s*```$/;
  const match = trimmedResponse.match(fenceRegex);

  if (match && match[1] !== undefined) { // Ensure group 1 was captured
    return match[1].trim(); // Return the captured group (the actual content), also trimmed
  }
  
  return responseString; // Return original string if no fences are matched
};
...
const AILayer = forwardRef<AILayerHandle, AILayerProps>((
  ...
  const fetchAuthToken = useCallback(async () => {
    ...
    try {
      ...
      setTokenExpiry(Date.now() + data.expires_in * 1000 - 60000); // 60 seconds buffer
      onLoading(false);
  ...
  }, [onLoading, onError]); // API_KEY, USERNAME, PASSWORD, AUTH_SCOPE are constants from .env
  ...
  const callAIAPIInternal = useCallback(async () => {
    ...
    if (!token) {
      // Error is handled by fetchAuthToken or getValidToken, which call onError and onLoading(false)
      return;
    }
    if (!API_KEY) { 
        ...
        console.error(errorMsg);
        onError(new Error(errorMsg));
        onLoading(false); // Ensure loading is stopped
        return;
    }
    ...
    try {
      ...
      if (resultData && typeof resultData.response === 'string') {
        rawResponseString = resultData.response;
      } else {
        ...
        rawResponseString = JSON.stringify(resultData, null, 2);
      }
      
      const cleanedResponseString = stripMarkdownFences(rawResponseString);
      onResult(cleanedResponseString);
