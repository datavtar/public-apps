...
const App: React.FC = () => {
  ...
  // Modified save data to localStorage function with improved error handling
  const saveDataToLocalStorage = (key: string, data: any) => {
    try {
      setDataUpdateStatus(prev => ({ ...prev, isSaving: true }));
      localStorage.setItem(key, JSON.stringify(data));
      const timestamp = new Date().toLocaleTimeString();
      setDataUpdateStatus({ isSaving: false, lastSaved: timestamp });
      return true;
    } catch (error) {
      console.error(`Error saving ${key} to localStorage:`, error);
      addNotification(`Failed to save data: ${(error as Error).message}`, 'error');
      setDataUpdateStatus(prev => ({ ...prev, isSaving: false }));
      return false;
    }
  };

  // Improved save data to localStorage when it changes
  useEffect(() => {
    if (movies.length > 0) {
      saveDataToLocalStorage('filmhubMovies', movies);
    }
  }, [movies]);

  useEffect(() => {
    if (authState.isAuthenticated) {
      saveDataToLocalStorage('filmhubAuth', authState);
    }
  }, [authState]);
  ...
  const refreshData = () => {
    try {
      ...
      addNotification(`Failed to refresh data: ${(error as Error).message}`, 'error');
    }
  };
  ...
  const handleSignUp = (e: React.FormEvent) => {
    ...
    try {
      ...
      // Explicitly save to localStorage for better error handling
      const savedSuccessfully = saveDataToLocalStorage('filmhubAuth', newAuthState);
      
      if (savedSuccessfully) {
        closeModal();
        addNotification(`Welcome, ${newUser.name}! You've successfully signed up as a ${newUser.role}.`, 'success');
      }
    } catch (error) {
      console.error('Error during sign up:', error);
      addNotification(`Sign up failed: ${(error as Error).message}`, 'error');
    }
  };
  ...
  const handleSignIn = (e: React.FormEvent) => {
    ...
    try {
      ...
      addNotification(`Sign in failed: ${(error as Error).message}`, 'error');
    }
  };
  ...
  const handleSignOut = () => {
    try {
      ...
      saveDataToLocalStorage('filmhubAuth', newAuthState);
      setCurrentSection('home');
      addNotification('You have been signed out successfully', 'success');
    } catch (error) {
      console.error('Error during sign out:', error);
      addNotification(`Sign out failed: ${(error as Error).message}`, 'error');
    }
  };
  ...
  const handleUploadMovie = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Verify all fields are valid before updating state
      if (!newMovie.title || !newMovie.description || !newMovie.category || isNaN(newMovie.year)) {
      ...
      // Explicitly save to localStorage here
      const savedSuccessfully = saveDataToLocalStorage('filmhubMovies', updatedMovies);
      
      if (savedSuccessfully) {
        closeModal();
        addNotification(`"${newMovie.title}" has been successfully uploaded!`, 'success');
      }
    } catch (error) {
      console.error('Error uploading movie:', error);
      addNotification(`Movie upload failed: ${(error as Error).message}`, 'error');
    }
  };
  ...
  const handleContactFormSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      addNotification(`Failed to send message: ${(error as Error).message}`, 'error');
    }
  };
  ...
  const handleDesignConfirmation = () => {
    ...
    addNotification('Design confirmed! Your new filmmaker banner is ready.', 'success');
    setTimeout(() => {
      ...
      <div className="container-fluid py-4">
        <div className="flex-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            >
              <RefreshCw size={16} />
              <span className="hidden sm:inline-block text-xs">Refresh</span>
            </button>
        ...
        <div className="md:hidden bg-black bg-opacity-95 text-white py-4">
          <nav className="container-fluid">
            <ul className="space-y-4">
              ...
              {/* Added Big Shot Challenge to mobile menu */}
              <li>
                <button 
                  onClick={() => setCurrentSection('challenge')} 
              ...
              <li>
                <button 
                  onClick={refreshData} 
                  className="block w-full text-left py-2 px-4 hover:bg-gray-800 flex items-center"
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2 text-gray-400">
              ...
              <li><button onClick={() => setCurrentSection('challenge')} className="hover:text-white">Big Shot Challenge</button></li>
  ...
  const renderHero = () => (
    <div className={styles.updatedHero}>
      <div className="absolute inset-0 bg-black bg-opacity-60"></div>
      <div className="relative z-10 container-fluid h-full flex flex-col justify-center">
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-between text-white">
          {/* Text content on the left */}
          <div className="mb-8 md:mb-0 md:mr-8 md:w-3/5 text-center md:text-left">
            <h2 className="text-xl md:text-2xl font-light mb-2 animate-fade-in">Introducing</h2>
            <h1 className={`text-3xl md:text-5xl lg:text-6xl font-bold mb-4 ${styles.animatedText}`}>
              ABC Talkies
            </h1>
            <p className="text-lg md:text-xl font-medium mb-6">
              World's First OTT Platform for Your Favorite Movies — Now with a Cinema Marketplace
            </p>
          ...
          <div className="md:w-2/5 flex justify-center">
            <div className={styles.bannerImageContainer}>
              <img 
                src="https://placehold.co/600x400/red/white?text=OTT+Platform" 
                alt="ABC Talkies OTT Platform" 
  ...
  // Mission statement with updated paragraphs
  const renderMissionSection = () => (
    <section className="py-12 bg-white dark:bg-gray-800">
      <div className="container-fluid">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className={`text-2xl md:text-3xl font-bold mb-6 ${styles.animatedHeading}`}>
            Our Mission: To Stream, Support, and Showcase True Cinema
          </h2>
          <div className="space-y-6 text-gray-700 dark:text-gray-300">
            <p className="text-lg leading-relaxed">
              Welcome to ABC Talkies. A new age digital cinema where you can watch online a vast selection of independent films, short films, new movies, old movies, biographies, trending movies and many more in all languages and genres. As a next gen film streaming platform we focus to serve each and every age group. Our original film OTT online platform present carefully chosen tales from around the globe, from award-winning authors to fearless up-coming voices. Enjoy top-notch entertainment on any device, at any time, and wherever you are, Whether you're looking for an OTT platform for new films or want to stream indie movies online, it doesn't matter if you favor drama, comedy, or documentaries. you will discover cinema beyond the mainstream with new, varied, and multilingual content in one location.
            </p>
            <p className="text-lg leading-relaxed">
              Our OTT services is not only next gen streaming platform but goes beyond - It is world's first online OTT platform for emerging filmmakers to upload their short films or untold stories to amuse audiences worldwide by showcasing their skills and inventiveness. Our Next-gen film streaming platform is where creativity thrives and stories are shared, regardless of whether you are an undiscovered filmmaker or work in any other field but have a passion for making movies as a part of your hobby. You can explore, upload, and compete here. It is made to highlight the finest in independent filmmaking by providing a platform for aspiring and seasoned filmmakers to stream, market, and make money from their work. That way we offer viewers a wide variety of new content that isn't available on popular platforms, ranging from streaming short films to untold stories with original features. By putting viewers and producers in direct contact, our digital cinema marketplace helps outstanding films receive the recognition they merit. Our OTT space offers quality and creativity on demand, whether you're looking to watch independent films, find up-coming talent, and enjoy storytelling without filters. Subscribe now world's best ott platform for endless fun!
            </p>
          </div>
        </div>
      </div>
  ...
  // Stream Now section heading
  const renderStreamNowHeading = () => (
    <div className="container-fluid py-8">
      <h2 className={`text-2xl md:text-3xl font-bold text-center ${styles.animatedHeading}`}>
        Stream Now
      </h2>
    </div>
  );
      ...
      <div className="container-fluid">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            {/* Updated paragraph for filmmaker section */}
            <p className="text-lg text-gray-600 dark:text-gray-300 mb-6">
              Join our community of independent filmmakers and showcase your work without any language barrier. Get discovered, connect with fans, and share your creative vision. Quickly upload your movie, reach a worldwide audience, and even make money off of it. We give you the resources and exposure you require, whether it's your debut short film or your next feature film that's ready to rock.
            </p>
    ...
    <section className={`${styles.uploadSection} py-16 text-white`}>
      <div className="container-fluid">
        <div className="max-w-3xl mx-auto text-center">
          ...
          {/* New section for Big Shot Challenge below upload button */}
          <div className="mt-16">
            <h3 className={`text-2xl font-bold mb-4 ${styles.animatedHeading}`}>
              If you've got a story to tell, this is your moment
            </h3>
            <p className="text-lg text-gray-200 leading-relaxed">
              The Big Shot Challenge honors storytelling from around the world. Filmmakers from all regions, languages, and backgrounds are welcome to enter this short film competition. Cinema is universal, so we encourage you to submit your film in the language of your choice. Gain international recognition, compete for thrilling rewards, and establish a connection with a creative audience. This is your chance to shine, regardless of the language you choose to tell your story in: English, Hindi, Tamil, Bengali, Marathi, or any other.
            </p>
          </div>
        </div>
      </div>
  ...
  const renderBigShotChallengeRegistration = () => (
    <section className={`${styles.challengeRegistrationSection} py-16 text-white`}>
      <div className="container-fluid">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className={`text-2xl md:text-3xl font-bold mb-4 ${styles.animatedHeading}`}>
            Ready to register in The Big Shot Challenge
          </h2>
          <p className="text-lg mb-8 text-gray-200">
            Accept the challenge and allow the world to hear your film.
          </p>
          <button 
            onClick={() => setCurrentSection('challenge')} 
            className="btn bg-white text-red-600 hover:bg-gray-100 shadow-lg flex items-center mx-auto"
          >
            <Trophy className="mr-2" /> Register Now
          </button>
        </div>
      </div>
    </section>
  ...
  const renderHomeSection = () => (
    <main className="mt-16 pt-8">
      {renderHero()}
      {/* Added new mission section */}
      {renderMissionSection()}
      {/* Added Stream Now heading */}
      {renderStreamNowHeading()}
      {renderTrendingMovies()}
      {renderFilmmakerPromo()}
      {renderUploadPromo()}
      {/* Added Big Shot Challenge Registration section */}
      {renderBigShotChallengeRegistration()}
      {renderUSP()}
    </main>
  );
  ...
  const renderFilmmakerSection = () => (
    <main className="mt-16 pt-16 pb-12">
      <div className="container-fluid">
        ...
        {/* Updated Hero section for filmmakers with enhanced cinema marketplace banner */}
        <div className={styles.filmmakerHero}>
          <div className="absolute inset-0 bg-black bg-opacity-70"></div>
          <div className="relative z-10 p-8 md:p-12 text-white max-w-2xl">
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-4">Welcome to the Cinema Marketplace</h2>
            <p className="text-lg mb-6">
              Connect with audiences, distributors, and fellow filmmakers in our innovative cinema marketplace. 
              Showcase your work, sell tickets to virtual premieres, and build your film career all in one place.
            </p>
              ...
              <button 
                onClick={() => {
                  if (!authState.isAuthenticated) {
                    ...
                    addNotification('Please create a filmmaker account to upload movies.', 'warning');
                  } else {
                    openModal('uploadMovie');
                  }
                }} 
                className="btn bg-red-600 hover:bg-red-700 text-white shadow-lg flex items-center justify-center"
              >
                <Upload className="mr-2" /> Upload Your Film
              </button>
              <button
            ...
            {designConfirmed && (
              <div className="mt-4 p-3 bg-green-600 bg-opacity-90 rounded-md shadow-lg animate-fade-in">
                <p className="text-white font-medium flex items-center">
                  ...
                  Design confirmed! Your new filmmaker banner is ready.
                </p>
              </div>
            )}
          </div>
          
          {/* Enhanced marketplace visual elements */}
          <div className={styles.marketplaceElements}>
            <div className={styles.filmTicket}></div>
  ...
  // Section for Big Shot Challenge
  const renderChallengeSection = () => (
    <main className="mt-16 pt-16 pb-12">
      <div className="container-fluid">
        <h1 className="text-3xl font-bold mb-8 flex items-center">
          <Trophy className="text-red-600 mr-2" /> Big Shot Challenge
        </h1>
        ...
        <div className={styles.challengeHero}>
          ...
          <div className="relative z-10 p-8 md:p-12 text-white max-w-2xl">
            ...
            <p className="text-lg mb-6">
              Join our Big Shot Challenge - a competition for filmmakers to create innovative short films under 10 minutes.
            ...
            <form onSubmit={handleUploadMovie} className="space-y-4 mt-4">
              ...
              <div className="modal-footer pt-4">
                ...
                <button 
                  ...
                  className="btn bg-red-600 hover:bg-red-700 text-white"
                >
                  <Save className="mr-2" size={16} /> Upload Film
                </button>
              </div>
            </form>
          </div>
    ...
    <div className="fixed bottom-4 right-4 z-50 flex flex-col space-y-2">
      {notifications.map(notification => {
        ...
        return (
          <div 
            key={notification.id} 
            className={`${bgColor} text-white px-4 py-3 rounded-lg shadow-lg flex items-center animate-fade-in`}
            role="alert"
          >
  ...
  // Render data status indicator (for showing when data is being saved/updated)
  const renderDataStatusIndicator = () => (
    <div className="fixed bottom-4 left-4 z-50 bg-gray-800 text-white px-3 py-2 rounded-lg shadow-lg text-sm flex items-center space-x-2">
      {dataUpdateStatus.isSaving ? (
        <>
          <RefreshCw size={14} className="animate-spin" />
          <span>Saving data...</span>
        </>
      ) : dataUpdateStatus.lastSaved ? (
        <>
          <CheckCircle size={14} className="text-green-400" />
          <span>Last saved: {dataUpdateStatus.lastSaved}</span>
        </>
      ) : null}
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {renderModals()}
      {renderNotifications()}
      {(dataUpdateStatus.isSaving || dataUpdateStatus.lastSaved) && renderDataStatusIndicator()}
