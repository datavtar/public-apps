...
import { Mic, MicOff, Settings, Play, Pause, Volume2, VolumeX, Trash2, MessageCircle, Key, Download, Upload, Wifi, WifiOff, Clock, AlertTriangle } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const MAX_RECORDING_DURATION = 30; // 30 seconds maximum recording
  const RECORDING_WARNING_TIME = 25; // 25 seconds - warn before auto-stop
  ...
  // Force stop recording - robust cleanup function
  const forceStopRecording = useCallback(() => {
    ...
    // Reset audio level and warnings
    setAudioLevel(0);
    setRecordingWarning(false);
    
    // Set state to processing if we have audio chunks, otherwise idle
    ...
    try {
      ...
      const connectionTimeout = setTimeout(() => {
        if (ws.readyState === WebSocket.CONNECTING) {
          ...
          setError('Connection timeout. Please check your API key and network connection.');
        }
      }, 10000);
  ...
  }, [isApiKeySet, settings.apiKey, startSessionTimer, stopSessionTimer, recordingState, forceStopRecording]);
  ...
  }, [messages, saveMessages, settings.autoPlay, recordingState]);
  ...
  const startRecording = useCallback(async () => {
    ...
    if (connectionState !== 'connected') {
      setError('Please wait for connection to establish before recording.');
      return;
    }
    ...
    try {
      ...
      timerRef.current = setInterval(() => {
        setRecordingTime(prev => {
          ...
          if (newTime >= RECORDING_WARNING_TIME && !recordingWarning) {
      ...
      // Set auto-stop timeout (30 seconds)
  ...
  }, [isApiKeySet, connectionState, monitorAudioLevel, forceStopRecording, recordingWarning]);
  ...
  }, [recordingState, forceStopRecording]);
  ...
  }, [disconnectWebSocket, forceStopRecording, recordingState]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-slate-900 dark:to-slate-800 theme-transition">
      <div className="container-fluid max-w-4xl">
        ...
        {/* Recording Warning */}
        {recordingWarning && recordingState === 'recording' && (
          <div className="alert alert-warning mb-6">
            <AlertTriangle className="w-5 h-5" />
            <span>Recording will auto-stop in {MAX_RECORDING_DURATION - recordingTime} seconds</span>
          </div>
        )}
          ...
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="relative">
              <button
                ...
                disabled={recordingState === 'processing' || !isApiKeySet || connectionState !== 'connected'}
                className={`
            ...
            <div className="text-center">
              <p className="text-lg font-medium text-gray-900 dark:text-white">
                {
                  recordingState === 'idle' && isApiKeySet && connectionState === 'connected'
                    ...
                    : recordingState === 'recording'
                    ? `Recording... ${formatTime(recordingTime)} / ${formatTime(MAX_RECORDING_DURATION)}`
                    : recordingState === 'processing'
              ...
              {recordingState === 'recording' && (
                <>
                  ...
                  <div className="mt-2 w-48 h-1 bg-gray-200 rounded-full overflow-hidden mx-auto">
                    <div 
                      className={`h-full transition-all duration-1000 ${
                        recordingTime >= RECORDING_WARNING_TIME ? 'bg-red-500' : 'bg-blue-500'
                      }`}
          ...
          <div className="flex-between">
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-slate-400">
                Real-time audio streaming with Gemini Live API (Auto-stop: 30s)
              </p>
            </div>
        ...
        {messages.length === 0 && (
          <div className="card-responsive bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
            ...
            <ul className="space-y-2 text-blue-800 dark:text-blue-200">
              ...
              <li className="flex items-start gap-2">
                <span className="w-2 h-2 bg-blue-500 rounded-full mt-2 flex-shrink-0" />
                Recording automatically stops after 30 seconds for reliability
              </li>
              <li className="flex items-start gap-2">
                <span className="w-2 h-2 bg-blue-500 rounded-full mt-2 flex-shrink-0" />
