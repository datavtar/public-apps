...
const App: React.FC = () => {
  ...
  const [tempSeats, setTempSeats] = useState<number>(4);
  const layoutRef = useRef<HTMLDivElement>(null);

  // Sample Data
  const sampleTables: Table[] = [
    { id: '1', number: 1, seats: 2, x: 100, y: 100, status: 'available' },
    { id: '2', number: 2, seats: 4, x: 250, y: 100, status: 'occupied', customerId: 'demo1', orderCode: '1001' },
  ...
  useEffect(() => {
    ...
    const savedCustomers = getFromLocalStorage('restaurant_customers', [demoCustomer]);
  ...
  const handleEditTableSeats = (tableId: string) => {
  ...
  const saveTableSeats = () => {
    if (editingTableId && tempSeats >= 1 && tempSeats <= 12) {
      setTables(prev => prev.map(table => 
        table.id === editingTableId ? { ...table, seats: tempSeats } : table
      ));
  ...
  const handleOrderCodeSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    let customer = customers.find(c => c.orderCode === orderCode);
    
    // Handle demo code 1001
    if (orderCode === '1001') {
      customer = demoCustomer;
      setCustomers(prev => {
        const existing = prev.find(c => c.orderCode === '1001');
        if (!existing) {
          return [...prev, demoCustomer];
        }
        return prev;
      });
    }
    ...
    if (customer) {
      ...
      alert('Invalid order code. Please check and try again. (Demo code: 1001)');
    }
  };
        ...
        <div className="max-w-md w-full">
          ...
          <div className="space-y-4">
            <button
              ...
              className="w-full card hover:shadow-lg transition-all duration-300 p-6 text-left group hover:bg-blue-50 border-2 hover:border-blue-200"
            >
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 mb-2 group-hover:text-blue-900 transition-colors">Manager Portal</h2>
                  <p className="text-gray-700 text-base font-medium group-hover:text-blue-700 transition-colors">Manage seating, orders, and restaurant operations</p>
                </div>
                <Settings className="h-10 w-10 text-orange-600 group-hover:text-blue-600 group-hover:scale-110 transition-all duration-300" />
              </div>
            </button>
            
            <button
              onClick={() => setAppMode('customer')}
              className="w-full card hover:shadow-lg transition-all duration-300 p-6 text-left group hover:bg-orange-50 border-2 hover:border-orange-200"
            >
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 mb-2 group-hover:text-orange-900 transition-colors">Customer Portal</h2>
                  <p className="text-gray-700 text-base font-medium group-hover:text-orange-700 transition-colors">Select seats, browse menu, and place orders</p>
                </div>
                <User className="h-10 w-10 text-orange-600 group-hover:text-orange-700 group-hover:scale-110 transition-all duration-300" />
              </div>
            </button>
          </div>
        </div>
        ...
        <main className="container-fluid py-6">
          ...
          {managerView === 'layout' && (
            <div className="space-y-6">
              ...
              {isEditingLayout && (
                <div className="card">
                  <h3 className="font-medium text-gray-900 mb-2">Edit Mode Instructions</h3>
                  <ul className="text-sm text-gray-600 space-y-1">
                    <li>• Drag tables to reposition them</li>
                    <li>• Click on a table to edit the number of seats</li>
                    <li>• Click the X button to delete a table</li>
              ...
              <div className="card">
                ...
                >
                  <p className="text-sm text-gray-500 mb-4">
                    {isEditingLayout ? 'Drag tables to reposition them or click to edit seats' : 'Current restaurant layout'}
                  </p>
                  
                  {tables.map(table => (
                    <div
                      key={table.id}
                      className={`absolute flex items-center justify-center rounded-lg text-white font-medium transition-transform hover:scale-105 ${
                        getStatusColor(table.status)
                      } ${isEditingLayout ? 'cursor-move' : ''} ${
                        editingTableId === table.id ? 'ring-4 ring-blue-300' : ''
                      }`}
                      style={getTableStyle(table)}
                      draggable={isEditingLayout && editingTableId !== table.id}
                      onDragStart={() => setDraggedTable(table)}
                      onClick={() => {
                        if (isEditingLayout && editingTableId !== table.id) {
                          handleEditTableSeats(table.id);
                        }
                      }}
                    >
                      <div className="text-center">
                        ...
                        {isEditingLayout && editingTableId !== table.id && (
                          <div className="text-xs mt-1">Click to edit</div>
                        )}
                      </div>
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-10">
                      <div className="bg-white rounded-lg p-6 shadow-xl">
                        <h3 className="text-lg font-semibold text-gray-900 mb-4">
                          Edit Table {tables.find(t => t.id === editingTableId)?.number} Seats
                        </h3>
                        <div className="mb-4">
                          ...
                          <input
                            ...
                            className="input"
                            value={tempSeats}
                            onChange={(e) => setTempSeats(parseInt(e.target.value) || 1)}
                          />
                          <p className="text-sm text-gray-500 mt-1">Enter a number between 1 and 12</p>
                        </div>
                        <div className="flex gap-2">
                          <button
                            onClick={saveTableSeats}
                            className="btn btn-primary"
                            disabled={tempSeats < 1 || tempSeats > 12}
                          >
                            Save
                          </button>
                          <button
              ...
              <div className="card mb-6">
                ...
                <form onSubmit={handleOrderCodeSubmit} className="flex gap-2">
                  <input
                    ...
                    placeholder="Enter 4-digit order code (try: 1001)"
                    value={orderCode}
                ...
                <div className="mt-2 p-2 bg-orange-50 rounded">
                  <p className="text-sm text-orange-700">
                    <strong>Demo Code:</strong> Use code <strong>1001</strong> to try the ordering system
                  </p>
                </div>
              </div>
