...
// Corrected recharts import - use standard names
...
import { 
  ...
  ArrowDownUp, Wheat, 
  PieChart as PieChartIcon, // Import lucide icon for Pie Chart
  BarChart3 as BarChartIcon // Import lucide icon for Bar Chart (BarChart3 is a common choice)
} from 'lucide-react';
...
// --- Constants (Gujarati) ---
const GUJARATI_TEXT = {
  APP_TITLE: 'હાર્વેસ્ટ સેલ્સ', // Changed to Harvest Sales in Gujarati
...
const App: React.FC = () => {
  const [isDarkMode, setIsDarkMode] = useState<boolean>(() => {
    if (typeof window !== 'undefined') {
      const savedMode = localStorage.getItem('harvestSalesDarkMode'); // Updated storage key
  ...
  const [sales, setSales] = useState<SaleItem[]>(() => {
    const savedSales = localStorage.getItem('harvestSales'); // Updated storage key
  ...
  useEffect(() => {
    ...
    if (isDarkMode) {
      ...
      localStorage.setItem('harvestSalesDarkMode', 'true'); // Updated storage key
    } else {
      document.documentElement.classList.remove('dark');
      localStorage.setItem('harvestSalesDarkMode', 'false'); // Updated storage key
    }
  }, [isDarkMode]);
  ...
  useEffect(() => {
    ...
    try {
      // Try to get data from the new storage location first
      let savedSales = localStorage.getItem('harvestSales');
      
      // If not found, check old storage location for migration
      if (!savedSales) {
        const oldSavedSales = localStorage.getItem('fruitVendorSales');
        if (oldSavedSales) {
          // Migrate data from old to new storage
          localStorage.setItem('harvestSales', oldSavedSales);
          savedSales = oldSavedSales;
          // Optionally clear old storage
          localStorage.removeItem('fruitVendorSales');
        }
      }
      ...
      if (savedSales) {
        ...
        localStorage.setItem('harvestSales', JSON.stringify(initialSales));
      }
    } catch (err) {
      console.error('Failed to load or parse sales data:', err);
      setError('ડેટા લોડ કરવામાં નિષ્ફળ.');
      localStorage.removeItem('harvestSales'); // Clear corrupted data
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    if (!loading) { // Avoid saving during initial load potentially wiping data
      try {
        localStorage.setItem('harvestSales', JSON.stringify(sales));
      } catch (err) {
    ...
    <div className={`min-h-screen flex flex-col theme-transition-all ${isDarkMode ? 'dark' : ''}`}>
      ...
      <header className="bg-white dark:bg-slate-800 shadow-md sticky top-0 z-[var(--z-sticky)] theme-transition-bg">
        <div className="container-wide flex-between py-3 px-4">
          <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-primary-600 dark:text-primary-400 flex items-center gap-2">
            <Wheat size={28} className="text-amber-500" /> {GUJARATI_TEXT.APP_TITLE}
          </h1>
      ...
      <main className="flex-grow container-wide py-6 px-4">
          ...
          <section className="mb-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
             <div className="card card-responsive theme-transition-all">
                <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
                    {/* Corrected: Use Lucide Icon */}
                    <BarChartIcon size={20} /> {GUJARATI_TEXT.SALES_PER_SEASON}
                </h3>
                <div className={styles.chartContainer}> {/* Apply fixed height */}
                   {/* Corrected: Use imported recharts components */}
            ...
            <div className="card card-responsive theme-transition-all">
                <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
                    {/* Corrected: Use Lucide Icon */}
                    <PieChartIcon size={20} /> {GUJARATI_TEXT.ITEM_TYPE_DISTRIBUTION}
                </h3>
                 <div className={styles.chartContainer}> {/* Apply fixed height */}
                   {/* Corrected: Use imported recharts components */}
                   <ResponsiveContainer width="100%" height={300}>
                    {/* Corrected: Use PieChart instead of RechartsPieChart */} 
                    <PieChart>
                      <Pie
        ...
        <section className="card card-responsive theme-transition-all">
          ...
          {/* Sales Table */}
          <div className={`${styles.tableResponsive} table-container theme-transition-all`}>
            <table className="table w-full">
              <thead>
                <tr>
                  ...
                  <th className="table-header hidden md:table-cell cursor-pointer" onClick={() => requestSort('type')}>
                  ...
                  <th className="table-header hidden sm:table-cell cursor-pointer" onClick={() => requestSort('pricePerUnit')}>
                  ...
                  <th className="table-header hidden sm:table-cell cursor-pointer" onClick={() => requestSort('date')}>
                    <div className="flex items-center gap-1">{GUJARATI_TEXT.DATE} {getSortIcon('date')}</div>
                  </th>
                  <th className="table-header hidden md:table-cell cursor-pointer" onClick={() => requestSort('season')}>
                  ...
                  filteredAndSortedSales.map((sale) => (
                    <tr key={sale.id} className="hover:bg-gray-50 dark:hover:bg-slate-800 theme-transition-bg">
                      <td className="table-cell font-medium text-gray-900 dark:text-white">
                        ...
                        <div className="md:hidden text-xs text-gray-500 dark:text-slate-400">{sale.type}</div>
                        <div className="sm:hidden text-xs text-gray-500 dark:text-slate-400 mt-1">
                          {formatDate(sale.date)} • {sale.season}
                        </div>
                      </td>
                      <td className="table-cell hidden md:table-cell">{sale.type}</td>
                      <td className="table-cell">{sale.quantity.toLocaleString('gu-IN')}</td>
                      <td className="table-cell hidden sm:table-cell">{formatCurrency(sale.pricePerUnit)}</td>
                      <td className="table-cell font-medium">{formatCurrency(sale.quantity * sale.pricePerUnit)}</td>
                      <td className="table-cell hidden sm:table-cell">{formatDate(sale.date)}</td>
...
const SaleFormModal: React.FC<SaleFormModalProps> = ({ modalState, onClose, onSave }) => {
  ...
  return (
    ...
    >
      <div
        className="modal-content theme-transition-all w-full max-w-lg mx-4" // Added mx-4 for better mobile spacing
      ...
      >
        <form onSubmit={handleSubmit}>
          ...
          <div className="modal-footer flex-wrap gap-3"> {/* Added flex-wrap for better mobile layout */} 
            <button
              type="button"
              onClick={onClose}
...
const DeleteConfirmationModal: React.FC<DeleteConfirmationModalProps> = ({ saleName, onClose, onConfirm }) => {
  ...
  return (
    ...
    >
      <div
        className="modal-content theme-transition-all max-w-sm mx-4" // Added mx-4 for better mobile spacing
