...
import { Search, Plus, User, Edit, Trash2, Filter, ChevronDown, ArrowLeftRight, Download, X, Check, Calendar, BookOpen, FileText, ChartBar, LogIn, Lock, UserRound, Upload, Eye, FileImage, FileVideo, FileAudio, Pill, FileJson } from 'lucide-react';
...
interface AssignmentReport {
  ...
  studentScores: { studentName: string; score: number }[];
}
...
const App: React.FC = () => {
  ...
  const [currentHomework, setCurrentHomework] = useState<HomeworkRecord | null>(null);
  ...
  const [performanceTimeframe, setPerformanceTimeframe] = useState<'week' | 'month' | 'semester'>('month');
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      if (user.role === 'student') {
        ...
        const studentData = sampleData.find((s) => s.email === user.email);
        if (studentData) {
          setActiveStudent(studentData.id);
          setActiveTab('grades');
        }
      }
    }
  ...
  // Generate assignment report when an assignment is selected
  useEffect(() => {
    if (selectedAssignment !== 'all') {
      generateAssignmentReport(selectedAssignment);
    } else {
  ...
  const filterStudents = () => {
    ...
    // If logged in as student, only show their data
    if (currentUser?.role === 'student') {
      filtered = filtered.filter(student => student.email === currentUser.email);
    }
  ...
  const generateSampleData = (): Student[] => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      for (let j = 0; j < 10; j++) {
        ...
        const hasSubmission = completed && Math.random() > 0.3; // 70% chance of having a submission if completed
        
        let submission: HomeworkSubmission | undefined = undefined;
        if (hasSubmission) {
          submission = {
            id: `submission-${i}-${j}`,
            textContent: `This is a sample submission for homework ${j + 1} by ${names[i]}. Lorem ipsum dolor sit amet, consectetur adipiscing elit.`,
            submittedAt: new Date(date.getTime() + Math.random() * 86400000).toISOString() // Random time on the same day
          };
        }
      ...
      // Generate hashed password (simplified for demo)
  ...
  // Generate report for a specific assignment
  const generateAssignmentReport = (assignmentId: string) => {
    // Get all grades for this assignment across all students
    const assignmentGrades: {studentId: string; studentName: string; grade: Grade}[] = [];
    
    students.forEach(student => {
      const grade = student.grades.find(g => g.id === assignmentId);
      if (grade) {
    ...
    const title = assignmentGrades[0].grade.title;
    const scores = assignmentGrades.map(item => (item.grade.score / item.grade.maxScore) * 100);
    const averageScore = scores.reduce((sum, score) => sum + score, 0) / scores.length;
    const highestScore = Math.max(...scores);
    const lowestScore = Math.min(...scores);
    const completionRate = (assignmentGrades.length / students.length) * 100;
    
    // Format student scores for the report
    const studentScores = assignmentGrades.map(item => ({
      studentName: item.studentName,
      score: (item.grade.score / item.grade.maxScore) * 100
  ...
  // Download assignment report as CSV
  const downloadAssignmentReport = () => {
    if (!assignmentReport) return;

    let csvContent = 'Assignment: ' + assignmentReport.title + '\n\n';
    csvContent += 'Statistics:\n';
    csvContent += `Average Score,${assignmentReport.averageScore.toFixed(2)}%\n`;
    csvContent += `Highest Score,${assignmentReport.highestScore.toFixed(2)}%\n`;
    csvContent += `Lowest Score,${assignmentReport.lowestScore.toFixed(2)}%\n`;
    csvContent += `Completion Rate,${assignmentReport.completionRate.toFixed(2)}%\n\n`;
    
    csvContent += 'Student Scores:\n';
    csvContent += 'Student,Score\n';
    
    assignmentReport.studentScores.forEach(item => {
      csvContent += `${item.studentName},${item.score.toFixed(2)}%\n`;
    });
    ...
    const link = document.createElement('a');
    link.setAttribute('href', url);
    link.setAttribute('download', `assignment-report-${assignmentReport.title}-${new Date().toISOString().split('T')[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  ...
  // Simple password hashing (IMPORTANT: this is not secure for production)
  const hashPassword = (password: string, salt: string): string => {
    // In a real application, use bcrypt or a similar library for proper hashing
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Check password
    const hashedPassword = hashPassword(loginPassword, student.salt);
  ...
  const handleStudentSubmit = (e: React.FormEvent) => {
    ...
    // Generate salt and hash password (default password for new students)
    const salt = Math.random().toString(36).substring(2, 15);
    const defaultPassword = 'password123'; // In a real app, generate a random password
  ...
  const handleHomeworkSubmission = (e: React.FormEvent) => {
    e.preventDefault();
    
    if (!activeStudent || !currentHomework) return;
    
    // Validate file if one was uploaded
    if (submissionFile) {
      const fileSize = submissionFile.size / 1024 / 1024; // Convert to MB
      ...
      if (!allowedTypes.includes(submissionFile.type)) {
        setFileUploadError('Only PDF, DOC, DOCX, and TXT files are allowed');
        return;
      }
    }
    
    // If no text or file, show error
    if (!submissionText && !submissionFile) {
      setFileUploadError('Please enter text or upload a file');
      return;
    }
    ...
    const homeworkIndex = students[studentIndex].homework.findIndex(h => h.id === currentHomework.id);
    ...
    const newSubmission: HomeworkSubmission = {
      ...
      textContent: submissionText || undefined,
    ...
    if (submissionFile) {
      ...
      reader.onload = (event) => {
        if (event.target && event.target.result) {
          ...
          // Update the homework record with the new submission
          updatedStudents[studentIndex].homework[homeworkIndex] = {
            ...updatedStudents[studentIndex].homework[homeworkIndex],
            completed: true,
      ...
      // Update with just text submission
      updatedStudents[studentIndex].homework[homeworkIndex] = {
        ...updatedStudents[studentIndex].homework[homeworkIndex],
        completed: true,
        submission: newSubmission
  ...
  // Submit homework
  const handleSubmitHomework = (record: HomeworkRecord) => {
    setCurrentHomework(record);
    setSubmissionText(record.submission?.textContent || '');
    setSubmissionFile(null);
    setFileUploadError('');
  ...
  const renderLoginPage = () => {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-blue-100 to-indigo-100 dark:from-gray-800 dark:to-gray-900 p-4">
        <div className="w-full max-w-md">
          <div className="card px-8 py-10 shadow-xl">
            <div className="text-center mb-6">
              <BookOpen size={48} className="mx-auto text-blue-600 dark:text-blue-400 mb-4" />
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              ...
              <button 
                type="submit" 
                className="btn btn-primary w-full py-3 flex-center"
              >
            ...
            <div className="mt-6 text-center text-sm text-gray-600 dark:text-gray-400">
              ...
              <p className="mt-1">Or any student email with password "password123"</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <table className="table">
            ...
            <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
              {filteredStudents.length === 0 ? (
                <tr>
                  <td colSpan={currentUser?.role === 'admin' ? 7 : 6} className="table-cell text-center py-8 text-gray-500 dark:text-gray-400">
                ...
                <table className="table">
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                    {sortedGrades.length === 0 ? (
                      <tr>
                        <td colSpan={currentUser?.role === 'admin' ? 5 : 4} className="table-cell text-center py-4 text-gray-500 dark:text-gray-400">
                          No grades recorded
                        </td>
                      </tr>
                    ) : (
                ...
                <table className="table">
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                    {sortedAttendance.length === 0 ? (
                      <tr>
                        <td colSpan={currentUser?.role === 'admin' ? 4 : 3} className="table-cell text-center py-4 text-gray-500 dark:text-gray-400">
                              ...
                              <div className="flex items-center space-x-2">
                                {currentUser?.role === 'admin' ? (
                                  ...
                                  <button 
                                    ...
                                    aria-label={hw.submission ? 'View Submission' : 'Submit Homework'}
                                  >
                                    {hw.submission ? (
                                      <>
                                        <Eye size={16} />
  ...
  const renderReports = () => {
    ...
    // Get all assignment IDs for the assignment report dropdown
    const allAssignments: { id: string; title: string }[] = [];
    
    // Get all assignments for the dropdown
    students.forEach(student => {
      student.grades.forEach(grade => {
        if (!allAssignments.some(a => a.id === grade.id)) {
          allAssignments.push({ id: grade.id, title: grade.title });
        }
      });
    });
    
    // Sort assignments by title
    allAssignments.sort((a, b) => a.title.localeCompare(b.title));
    
    // Get performance data over time based on selected timeframe
    const getTimeframeData = () => {
      ...
      if (performanceTimeframe === 'week') {
        ...
        startDate = new Date(today);
        startDate.setMonth(today.getMonth() - 4); // Assuming a semester is about 4 months
      }
      
      const startTimestamp = startDate.getTime();
      
      // Get data for each student
      const performanceData: { name: string; grades: {date: string; score: number}[] }[] = [];
      
      // If we're viewing as a student, only show current student's data
      const selectedStudents = currentUser?.role === 'student' && activeStudent
        ? [getStudentById(activeStudent)].filter(Boolean) as Student[]
        : students;
      
      selectedStudents.forEach(student => {
        const filteredGrades = student.grades
          .filter(grade => new Date(grade.date).getTime() >= startTimestamp)
    ...
    const prepareTimelineData = () => {
      ...
      timeframeData.forEach(student => {
        student.grades.forEach(grade => {
          allDates.add(grade.date);
        });
      });
      ...
      return sortedDates.map(date => {
        ...
        timeframeData.forEach(student => {
          const gradeForDate = student.grades.find(g => g.date === date);
          if (gradeForDate) {
            dataPoint[student.name] = gradeForDate.score;
          } else {
            dataPoint[student.name] = null; // No data for this date
          }
        });
    ...
    return (
      <div className="space-y-6">
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                onChange={(e) => setPerformanceTimeframe(e.target.value as 'week' | 'month' | 'semester')}
            ...
            {timelineData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Tooltip />
                  <Legend />
                  {timeframeData.map((student, index) => (
                    <Line
                      key={student.name}
                      type="monotone"
        ...
        {currentUser?.role === 'admin' && (
          <div className="card">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-semibold">Assignment Report</h3>
              <div className="flex items-center gap-2">
                <select
                  className="input-sm"
                ...
                >
                  <option value="all">Select an Assignment</option>
                ...
                {assignmentReport && (
                  <button 
                    onClick={downloadAssignmentReport}
                    className="btn btn-sm bg-gray-200 text-gray-700 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 flex items-center gap-1"
            ...
            {assignmentReport ? (
              <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="card-sm bg-blue-50 dark:bg-blue-900">
                    <h4 className="text-sm font-medium text-blue-800 dark:text-blue-200">Average Score</h4>
                    <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{assignmentReport.averageScore.toFixed(1)}%</p>
                  </div>
                  <div className="card-sm bg-green-50 dark:bg-green-900">
                    <h4 className="text-sm font-medium text-green-800 dark:text-green-200">Highest Score</h4>
                    <p className="text-2xl font-bold text-green-600 dark:text-green-400">{assignmentReport.highestScore.toFixed(1)}%</p>
                  </div>
                  <div className="card-sm bg-red-50 dark:bg-red-900">
                    <h4 className="text-sm font-medium text-red-800 dark:text-red-200">Lowest Score</h4>
                    <p className="text-2xl font-bold text-red-600 dark:text-red-400">{assignmentReport.lowestScore.toFixed(1)}%</p>
                  </div>
                  <div className="card-sm bg-purple-50 dark:bg-purple-900">
                    <h4 className="text-sm font-medium text-purple-800 dark:text-purple-200">Completion Rate</h4>
                    <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{assignmentReport.completionRate.toFixed(1)}%</p>
                  </div>
                </div>
                
                <div className="space-y-4">
                  <h4 className="text-lg font-medium">Student Scores</h4>
                  <div className="overflow-x-auto">
                    <table className="table">
                      <thead>
                        <tr>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        {assignmentReport.studentScores.map((item, index) => (
                          <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td className="table-cell font-medium">{item.studentName}</td>
                            <td className="table-cell">
                              <span className={`badge ${item.score >= 90 ? 'badge-success' : item.score >= 70 ? 'badge-info' : 'badge-error'}`}>
              ...
              <div className="text-center py-8">
                <p className="text-gray-500 dark:text-gray-400">Select an assignment to view its report</p>
              </div>
            )}
          </div>
        )}
  ...
  // Render homework submission modal
  const renderHomeworkSubmissionModal = () => {
    if (!isSubmittingHomework || !currentHomework) return null;
    
    const existingSubmission = currentHomework.submission;
    const isViewingOnly = existingSubmission !== undefined;
    ...
    return (
      <div className="modal-backdrop" onClick={() => {
        ...
        <div className="modal-content" onClick={(e) => e.stopPropagation()}>
          <div className="modal-header">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white" id="homework-submission-modal-title">
              {isViewingOnly ? 'View Homework Submission' : 'Submit Homework'}
            </h3>
            <button 
              className="text-gray-400 hover:text-gray-500 dark:text-gray-300 dark:hover:text-gray-200" 
              onClick={() => {
          ...
          <div className="p-1">
            <h4 className="font-medium mb-2">{currentHomework.title}</h4>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">Due date: {currentHomework.date}</p>
            ...
            {isViewingOnly ? (
              <div className="space-y-4">
                ...
                {existingSubmission.textContent && (
                  <div className="space-y-2">
                    <h5 className="text-sm font-medium">Submission Text:</h5>
                ...
                {existingSubmission.fileUpload && (
                  <div className="space-y-2">
                    <h5 className="text-sm font-medium">Uploaded File:</h5>
                    <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-md">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          {existingSubmission.fileUpload.fileType.includes('pdf') ? (
                            <FileText size={24} className="text-red-500" />
                          ) : existingSubmission.fileUpload.fileType.includes('word') ? (
                            <FileText size={24} className="text-blue-500" />
                          ) : existingSubmission.fileUpload.fileType.includes('text') ? (
                            <FileText size={24} className="text-gray-500" />
                          ) : (
                            <FileText size={24} className="text-gray-500" />
                          )}
                          <div>
                            <p className="text-sm font-medium">{existingSubmission.fileUpload.fileName}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                              {(existingSubmission.fileUpload.fileSize / 1024 / 1024).toFixed(2)} MB
                            </p>
                          </div>
                        </div>
                        
                        <a 
                          href={existingSubmission.fileUpload.fileContent} 
                          download={existingSubmission.fileUpload.fileName}
                          className="btn btn-sm bg-blue-100 text-blue-700 hover:bg-blue-200 dark:bg-blue-900 dark:text-blue-200 dark:hover:bg-blue-800 flex items-center gap-1"
              ...
              <form onSubmit={handleHomeworkSubmission} className="space-y-4">
                <div className="form-group">
                  <label className="form-label" htmlFor="submission-text">Submission Text</label>
                  <textarea 
                    id="submission-text" 
                    className="input" 
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="submission-file">Upload File (optional)</label>
                  <p className="text-xs text-gray-600 dark:text-gray-400 mb-2">
                    Accepts PDF, DOC, DOCX, and TXT files (max 10MB)
                  </p>
                  <input 
                    type="file" 
                    id="submission-file" 
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {submissionFile && (
                      <div className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                        {submissionFile.type.includes('pdf') ? (
                          <FileImage size={20} className="text-red-500" />
                        ) : submissionFile.type.includes('word') ? (
                          <FileText size={20} className="text-blue-500" />
                        ) : submissionFile.type.includes('text') ? (
                  ...
                  {fileUploadError && (
                    <p className="form-error mt-2">{fileUploadError}</p>
                  )}
                </div>
            ...
            {currentUser && (
              <div className="flex items-center gap-2">
                ...
                <button 
                  ...
                  className="btn btn-sm bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 flex items-center gap-1"
                  aria-label="Logout"
                >
