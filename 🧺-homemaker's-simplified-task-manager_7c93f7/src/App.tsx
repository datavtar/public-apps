...
import {
  ...
  CalendarDays, Tags, PieChart as PieChartIcon,
  CheckSquare, Square, Info, SortAsc, SortDesc, AlertTriangle
} from 'lucide-react';
...
const App: React.FC = () => {
    ...
    try {
      ...
      if (savedTodos) {
        ...
        return parsedTodos.map((todo: any) => ({
          ...
          completed: todo.completed ?? false, // Ensure completed exists
  ...
  const getPriorityValue = useCallback((priority: Priority): number => {
    switch (priority) {
  ...
  const getPriorityIcon = useCallback((priority: Priority, size: number = 16) => {
    switch (priority) {
  ...
  const filteredAndSortedTodos = useMemo(() => {
    return todos
      .filter(todo => {
        if (!todo) return false; // Ensure todo object exists
      ...
      .sort((a, b) => {
        ...
        switch (sortBy) {
          case 'dueDate':
            // Handle potential null/undefined or invalid dates safely
          ...
          case 'priority':
            ...
            break;
          case 'text':
            comparison = (a.text ?? '').localeCompare(b.text ?? ''); // Handle potential null/undefined text
            break;
          case 'createdAt':
          default:
            comparison = (b.createdAt ?? 0) - (a.createdAt ?? 0); // Default: Newest first, handle potential null/undefined
            break;
        }
  ...
  }, [todos, searchTerm, filter, sortBy, sortDirection, getPriorityValue]); // Added getPriorityValue dependency
  ...
  const priorityChartData: ChartData[] = useMemo(() => {
      ...
      // Ensure all priorities are present, even if count is 0, for consistent coloring
      const fullCounts = {
          [Priority.High]: counts[Priority.High] || 0,
          [Priority.Medium]: counts[Priority.Medium] || 0,
          [Priority.Low]: counts[Priority.Low] || 0,
      };
      return [
          { name: 'High', value: fullCounts[Priority.High] },
          { name: 'Medium', value: fullCounts[Priority.Medium] },
          { name: 'Low', value: fullCounts[Priority.Low] },
  ...
  const handleAddTodo = useCallback((event: FormEvent<HTMLFormElement>) => {
    ...
    setTodos(prevTodos => [newTodo, ...prevTodos]); // Add new todo to the beginning using functional update
    setNewTodoText('');
    setNewTodoPriority(Priority.Medium);
    setNewTodoDueDate('');
    setNewTodoTags(''); // Clear tags input
  }, [newTodoText, newTodoPriority, newTodoDueDate, newTodoTags, parseTags]);
  ...
  const confirmDelete = useCallback(() => {
    if (deletingMultiple) {
       setTodos(prevTodos => prevTodos.filter(todo => !selectedTodos.has(todo.id)));
    ...
    closeDeleteModal();
  }, [deletingMultiple, deletingTodoId, selectedTodos, closeDeleteModal]);
  ...
  const handleEditInputChange = useCallback((event: ChangeEvent<HTMLInputElement>) => {
    setEditText(event.target.value);
  }, []);

  const handleEditPriorityChange = useCallback((event: ChangeEvent<HTMLSelectElement>) => {
    setEditPriority(event.target.value as Priority);
  }, []);

  const handleEditDueDateChange = useCallback((event: ChangeEvent<HTMLInputElement>) => {
    setEditDueDate(event.target.value);
  }, []);

  const handleEditNotesChange = useCallback((event: ChangeEvent<HTMLTextAreaElement>) => {
    setEditNotes(event.target.value);
  }, []);

  const handleEditTagsChange = useCallback((event: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSaveEdit = useCallback(() => {
    ...
    closeEditModal();
  }, [editingTodo, editText, editPriority, editDueDate, editNotes, editTags, parseTags, closeEditModal]);

  const handleEditKeyDown = useCallback((event: KeyboardEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    const targetElement = event.target as HTMLElement;
    // Allow Enter in textarea, but save on Ctrl+Enter or Cmd+Enter
    if (event.key === 'Enter' && (event.ctrlKey || event.metaKey) && targetElement.tagName === 'TEXTAREA') {
        event.preventDefault(); // Prevent newline in textarea
        handleSaveEdit();
    } else if (event.key === 'Enter' && targetElement.tagName !== 'TEXTAREA' && targetElement.tagName !== 'SELECT') {
  ...
  const handleSelectTodo = useCallback((id: string) => {
    setSelectedTodos(prevSelection => {
  ...
  const isAllSelected = useMemo(() => {
      if (!filteredAndSortedTodos || filteredAndSortedTodos.length === 0) return false;
      const visibleIds = new Set(filteredAndSortedTodos.map(t => t.id));
      return [...selectedTodos].every(id => visibleIds.has(id)) && selectedTodos.size === visibleIds.size;
  ...
  const handleBulkComplete = useCallback(() => {
      if (selectedTodos.size === 0) return;
      setTodos(prevTodos => prevTodos.map(todo => selectedTodos.has(todo.id) ? { ...todo, completed: true } : todo));
  ...
  useEffect(() => {
    try {
      localStorage.setItem('todos_homemaker_v3', JSON.stringify(todos));
    } catch (error) {
  ...
  }, [editingTodo, deletingTodoId, deletingMultiple, closeEditModal, closeDeleteModal]); // Added stable callbacks
      ...
      <div className="container-wide mx-auto px-4 py-8 theme-transition-bg">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                    ...
                                    <PieChart>
                                        <Pie
                                            ...
                                            label={({ cx, cy, midAngle, innerRadius, outerRadius, percent, name }) => {
                                                // Check if percent is valid before using it
                                                ...
                                                return percent > 0.05 ? (
                                                    <text x={x} y={y} fill="white" textAnchor={x > (cx ?? 0) ? 'start' : 'end'} dominantBaseline="central" fontSize="10px">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="mb-4 card card-sm p-3 theme-transition-all shadow-sm">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center">
                    ...
                    <div className="flex gap-2 items-center flex-wrap justify-center md:justify-end md:col-span-1">
                        ...
                        <button
                            onClick={() => setSortDirection(prevDir => prevDir === 'asc' ? 'desc' : 'asc')}
                            ...
                            title={sortDirection === 'asc' ? 'Sort Descending' : 'Sort Ascending'} /* Updated title based on current state */
                        >
            ...
            {filteredAndSortedTodos.length > 0 && (
                <div className="flex items-center px-4 py-2 mb-2 text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider bg-gray-50 dark:bg-slate-800 rounded-t-md theme-transition-all">
                    <input
                        ...
                        disabled={filteredAndSortedTodos.length === 0} // Disable if no tasks visible
                    />
            ...
            {filteredAndSortedTodos.length > 0 ? (
                ...
                const isValidDueDate = todo.dueDate && isValid(parseISO(todo.dueDate));
                const isOverdue = isValidDueDate ? !todo.completed && isPast(parseISO(todo.dueDate as string)) : false;
                return (
                <div
                    key={todo.id}
                ...
                >
                    ...
                    <div className="flex-grow min-w-0">
                        ...
                        {/* Notes */} 
                         {todo.notes && (
                            <p className="text-xs text-gray-500 dark:text-slate-400 mb-1 break-words" title={todo.notes}>
                        ...
                        {isValidDueDate && (
                            ...
                            {format(parseISO(todo.dueDate as string), 'MMM d, yyyy')}
                            </span>
                        )}
        ...
        {editingTodo && (
          <div
            ...
            onClick={closeEditModal} // Use stable callback
            ref={modalRef}
            role="dialog"
            aria-modal="true"
          ...
          >
            ...
            >
              ...
              <div className="modal-footer">
                ...
                <button
                  onClick={handleSaveEdit} // Use stable callback
        ...
        {(deletingTodoId || deletingMultiple) && (
          <div
            className="modal-backdrop fade-in"
            onClick={closeDeleteModal} // Use stable callback
          ...
          >
            ...
            >
              ...
              <div className="modal-footer">
                ...
                <button
                  onClick={confirmDelete} // Use stable callback
