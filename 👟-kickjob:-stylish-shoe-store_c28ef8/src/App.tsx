...
const App: React.FC = () => {
  // Auth context import
  const { currentUser, logout } = { currentUser: null, logout: () => {} }; // Will be replaced by actual auth context
  ...
  const handleAIChat = () => {
    ...
    const contextualPrompt = `You are a helpful customer service assistant for KickJob shoe store. 
    Current products available: ${products.map(p => `${p.name} - $${p.price} (${p.category})`).join(', ')}.
    Customer question: ${aiPrompt}
    Please provide helpful, friendly assistance about our shoes, sizing, orders, or general questions.`;
        ...
        <div className="bg-white rounded-lg shadow-xl w-80 h-96 flex flex-col">
          ...
          <div className="flex-1 p-4 overflow-y-auto">
            ...
            {aiError && (
              <div className="mb-4 p-3 bg-red-50 rounded-lg">
                <p className="text-sm text-red-600">Sorry, I encountered an error. Please try again.</p>
              </div>
            )}
            {aiLoading && (
          ...
          <div className="p-4 border-t">
            <div className="flex gap-2">
              ...
              />
              <button onClick={handleAIChat} className="btn btn-primary" disabled={aiLoading}>
                Send
              </button>
            </div>
          </div>
