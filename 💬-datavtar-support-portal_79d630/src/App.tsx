...
import { 
  ...
  Lock, Unlock, Key, Shield, HelpCircle, X, ChevronRight
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const tourSteps: TourStep[] = [
    {
      ...
      content: 'This is your dashboard where you can see an overview of all your support tickets. Click on the stat boxes to filter tickets by status.',
      placement: 'bottom'
    },
    {
      target: 'tickets-tab',
      title: 'View Your Tickets',
      content: 'Here you can view all your support tickets. Use the search bar and filters to find specific tickets quickly.',
      placement: 'bottom'
    },
    {
      target: 'create-tab',
      title: 'Create New Tickets',
      content: 'Need help? Click here to create a new support ticket. Our AI can help analyze your issue and suggest the right category and priority.',
      placement: 'bottom'
    },
    {
      target: 'settings-tab',
      title: 'Customize Your Experience',
      content: 'Visit settings to toggle dark mode, export your data, and manage your preferences.',
      placement: 'bottom'
    }
  ];
  ...
  useEffect(() => {
    ...
    const savedTourCompleted = localStorage.getItem('datavtar_tour_completed');
    ...
    // Check if tour should be shown for new customers
    if (savedTourCompleted === 'true') {
      setTourCompleted(true);
    } else if (viewMode === 'customer' && !savedTourCompleted) {
      // Show tour after a brief delay for new customers
      setTimeout(() => {
        setShowTour(true);
      }, 1000);
    }
...
- If customer details are not clear, use placeholder values`;
  ...
  const completeTour = () => {
    setShowTour(false);
    setTourCompleted(true);
    localStorage.setItem('datavtar_tour_completed', 'true');
  };
  ...
  }, [showPasswordModal, showTicketModal, showConfirmDialog, showNotifications, showTour, showEmailToTicketForm]);
            ...
            <div className="flex items-center gap-4 animate-slide-in-left">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#548B99]">Customer Support Portal v1.4.0</p>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 animate-fade-in">
          <div className="relative h-full">
            {tourSteps.map((step, index) => {
              ...
              const isRight = step.placement === 'right';
              
              return (
                <div key={index} className="absolute">
                  {/* Highlight overlay */}
                  <div 
                    className="absolute bg-white dark:bg-slate-800 rounded-xl z-10 animate-pulse"
                    style={{
                      top: rect.top - 4,
                      left: rect.left - 4,
                      width: rect.width + 8,
                      height: rect.height + 8,
                      boxShadow: '0 0 0 4px rgba(84, 139, 153, 0.5), 0 0 0 9999px rgba(0, 0, 0, 0.5)'
                    }}
                  />
                  
                  {/* Tour popup */}
                  <div 
                    className="absolute bg-white dark:bg-slate-800 rounded-2xl shadow-2xl p-6 max-w-sm border border-slate-200 dark:border-slate-700 z-20 animate-scale-in"
                    style={{
                      top: isBottom ? rect.bottom + 20 : rect.top - 200,
                      left: isRight ? rect.right + 20 : Math.max(20, rect.left - 200),
                      minWidth: '300px'
                    }}
                  >
                    <div className="flex items-start gap-3 mb-4">
                      <div className="w-8 h-8 bg-[#548b99] rounded-full flex items-center justify-center flex-shrink-0">
                        <span className="text-white text-sm font-medium">{tourStep + 1}</span>
                      </div>
                      <div className="flex-1">
                        <h4 className="font-semibold text-[#1F2E3D] dark:text-[#e7f7f7] mb-2">{step.title}</h4>
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        {tourSteps.map((_, i) => (
                          <div 
                            ...
                            className={`w-2 h-2 rounded-full transition-all duration-300 ${
                              i === tourStep ? 'bg-[#548b99]' : 'bg-slate-300 dark:bg-slate-600'
                            }`}
                          />
                        ))}
                      </div>
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={skipTour}
                          className="px-3 py-1 text-sm text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition-colors duration-200"
                        >
                          Skip Tour
                        </button>
                        <button
                          onClick={nextTourStep}
                          className="btn bg-[#548b99] text-white hover:bg-[#3d6b75] rounded-lg text-sm transition-all duration-300 transform hover:scale-105 flex items-center gap-1"
                        >
