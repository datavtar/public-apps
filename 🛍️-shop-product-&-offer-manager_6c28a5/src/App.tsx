...
function App() {
  ...
  const handleAiProductAnalysis = (file: File) => {
    const prompt = `Analyze this product image and extract detailed information. Return JSON with keys: "name", "description", "category", "estimatedPrice", "tags", "features". Focus on identifying the product type, key features, and provide a detailed description suitable for e-commerce. Provide prices in Indian Rupees (INR).`;
  ...
  const handleAiBulkDescription = () => {
    ...
    const prompt = `Generate a compelling product description for e-commerce based on these details: ${aiPrompt}. Return JSON with keys: "description", "features", "benefits", "suggestedTags". Make it marketing-friendly and highlight key selling points. Consider this is for the Indian market.`;
  ...
  const handleDownloadTemplate = () => {
    const headers = ['Name', 'Description', 'Price', 'Category', 'Stock', 'SKU', 'Supplier', 'Tags'];
    const sampleRow = ['Sample Product', 'Product description here', '2999.99', 'Electronics', '100', 'SKU001', 'Supplier Name', 'tag1; tag2'];
        ...
        <div className="stat-card">
          <div className="flex-between">
            <div>
              ...
              <div className="stat-value">{formatCurrency(analytics.totalValue, storeSettings.currency)}</div>
            </div>
              ...
              <div className="flex-between">
                <div>
                  <span className="text-xl font-bold text-green-600">
                    {formatCurrency(product.price, storeSettings.currency)}
                  </span>
                  {product.originalPrice && product.originalPrice > product.price && (
                    <span className="text-sm text-gray-500 line-through ml-2">
                      {formatCurrency(product.originalPrice, storeSettings.currency)}
                    </span>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-1">
                  {product.offers.filter(o => o.isActive).map(offer => (
                    <span key={offer.id} className="badge badge-warning text-xs">
                      {offer.type === 'percentage' ? `${offer.value}% OFF` : `${formatCurrency(offer.value, storeSettings.currency)} OFF`}
                    </span>
                  ))}
                </div>
              )}
        ...
        <div className="card">
          ...
          <div className="space-y-3">
            ...
            <div className="flex-between">
              ...
              <span className="font-medium">{formatCurrency(analytics.totalValue, storeSettings.currency)}</span>
            </div>
            <div className="flex-between">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          ...
          <div className="space-y-4">
            ...
            <button
              onClick={() => {
                const shouldDelete = window.confirm('Are you sure? This will delete all products and cannot be undone.');
                if (shouldDelete) {
                  setProducts([]);
