...
const App: React.FC = () => {
 ...
 <td className="table-cell">
 <button onClick={() => toggleComplete(todo.id)} className="focus:outline-none">
 <Check className={
 todo.completed
 ? 'h-5 w-5 text-green-500 dark:text-green-400'
 : 'h-5 w-5 text-gray-500 dark:text-slate-400'
 }
 role="checkbox"
 ...
 <button className="btn btn-success" onClick={updateTodo} role="button" name="updateTodo">
 Update
 </button>
 <button className="btn btn-cancel" onClick={() => setEditTodoId(null)} role="button" name="cancelEdit">
 Cancel
 </button>
 </div>
 ) : (
 <div className="flex gap-2">
 <button className="btn btn-edit" onClick={() => startEdit(todo.id, todo.task, todo.dueDate)} role="button" name="editTodo">
 <Edit className="h-5 w-5" />
 </button>
 <button className="btn btn-delete" onClick={() => deleteTodo(todo.id)} role="button" name="deleteTodo">
